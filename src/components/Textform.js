    ...
    return (
        <>
            <div className="container mt-5" style={{ color: mode === 'dark' ? 'white' : 'black' }}>
                <h1>{Heading} </h1>
                <div className="mb-3 ">
                    <textarea value={text} onChange={handleOnChange} style={{ backgroundColor: mode === 'dark' ? 'grey' : '#e1e1e1', color: mode === 'dark' ? 'white' : 'black' }} className="form-control" id="exampleFormControlTextarea1" rows="8"></textarea>
                </div>
                <button onClick={handleUpClick} className="btn btn-primary mx-1 my-1">Convert to upercase</button>
                {notification2 && <div className="notification">Converted to uper case!</div>}
                <button onClick={convertToLower} className="btn btn-primary mx-1 my-1">Convert to lowercase</button>
                {notification3 && <div className="notification">Converted to lower case!</div>}
                <button onClick={handleClearClick} className="btn btn-primary mx-1 my-1">clear</button>
                ...
                <button onClick={handleEmailCopyClick} className="btn btn-primary mx-1 my-1">Copy Email</button>
                {email && <div className="email">Email Copied to clipboard!</div>}
                <button onClick={handleRemoveExtrasapcesClick} className="btn btn-primary mx-1 my-1">Remove Extra Spaces</button>
            ...
            <div className="container" style={{ color: mode === 'dark' ? 'white' : 'black' }}>
                ...
                <p className='fw-bold'><span className="text-success">{text.split(" ").filter((element)=>{return element.length !==0}).length}</span> words and <span className="text-success">{text.length === 0 ? '0' : text.length}</span> characters </p>
                <p className='fw-bold'><span className="text-success">{0.008 *text.split(" ").filter((element)=>{return element.length !==0}).length}</span> minutes read </p>
                <h2>Preview</h2>
